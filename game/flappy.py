            ...
            if r.keypoints is not None:
                ...
                if len(kps) > 0:
                    ...
                    if len(person) > 10:
                        ...
                        # Check that shoulders and wrists are properly detected in the frame
                        if ls[0] > 0 and rs[0] > 0 and lw[0] > 0 and rw[0] > 0:
                            
                            # Note: In OpenCV, Y=0 is the top of the screen. 
                            # A smaller Y value means higher up.
                            
                            # Step 1: Wings Up (Wrists are higher than shoulders)
                            if lw[1] < ls[1] - 40 and rw[1] < rs[1] - 40:
                                arms_up = True
                                
                            # Step 2: Wings Down (Wrists go back below shoulders)
                            elif lw[1] > ls[1] + 20 and rw[1] > rs[1] + 20:
                                if arms_up:  # Only jump if arms were previously raised
                                    current_jump = -JUMP_STRENGTH if is_gravity_reversed else JUMP_STRENGTH
                                    ufo_vy = current_jump
                                    arms_up = False  # Reset state so you have to lift them again
...
def show_analytics():
    ...
    with open("graph/Flap/1.txt", "a") as f: f.write(f"{final_score}\n")
    ...
    history_1 = load_data("graph/Flap/1.txt")
    history_2 = load_data("graph/Flap/2.txt")
